...
def fix_image(model):
    for name, param in model.named_parameters():
        ...
        elif ("visual.ln_post" in name) or ("visual.proj" in name) or ("visual.class_embedding" in name):
            param.requires_grad = True
        elif ("visual." in name) and ("temporal_" not in name):
            param.requires_grad = False
        else:
            continue
...
def build_optimizer(config, model):
    ...
    if config.MODEL.FIX_IMAGE:
        fix_image(model)
        optimizer = optim.AdamW(clip_parameters + temporal_parameters, betas=(0.9, 0.98), eps=1e-8,)
    else:
        optimizer = optim.AdamW(clip_parameters + temporal_parameters, betas=(0.9, 0.98), eps=1e-8,)
