...
def main(config): 
    ...
    model, _ = claver.load(config.MODEL.PRETRAINED, config.MODEL.ARCH,
                         device="cpu", jit=False, 
                         T=config.DATA.NUM_FRAMES, 
                         droppath=config.MODEL.DROP_PATH_RATE, 
                         use_checkpoint=config.TRAIN.USE_CHECKPOINT, 
                         use_cache=config.MODEL.FIX_TEXT,
                         logger=logger, mask_mode=config.MODEL.MASK_MODE,
                        )
    model = model.cuda()
    ...
    text_labels = generate_text(train_data) # [A, num_classes, content_length]
...
def train_one_epoch(epoch, model, criterion, optimizer, lr_scheduler, train_loader, text_labels, config, mixup_fn):
    ...
    texts = text_labels.cuda(non_blocking=True) # [A, num_classes, content_length]
    A, C, _ = texts.shape
    K = 5 # optional
    for idx, batch_data in enumerate(train_loader):

        rnd = torch.randint(0, A, (K,))
        texts_ = texts[[rnd]]
        ...
        output = model(images, texts_) # [B, K, num_classes]
        output = output.reshape(-1, C) # [B*K, num_classes]
        label_id = label_id.unsqueeze(1).repeat(1, K, 1) # [B, K, C]
        label_id = label_id.reshape(-1, C) # [B*K, C]
    ...
    with torch.no_grad():
        ...
        for idx, batch_data in enumerate(val_loader):
            ...
            for i in range(n):
                ...
                output = model(image_input, text_inputs) # [b, A, C]
                output = output.reshape(b*A, -1) # [b*A, C]

                similarity = output.softmax(dim=-1)
                similarity = similarity.reshape(b, A, -1)
            ...
            acc1, acc5 = 0, 0
            for i in range(b):
                if indices_1[i] // A == label_id[i]:
                    acc1 += 1
                if label_id[i] // A in indices_5[i]:
                    acc5 += 1
