...
class VideoDataset(BaseDataset):
    def __init__(self, ann_file, pipeline, labels_file, descriptions_file=None, start_index=0, **kwargs):
    ...
    def classes(self):
        ...
        if self.descriptions_file is not None:
            augment_texts0 = pd.read_csv(self.descriptions_file[0], sep=';')
            augment_texts1 = pd.read_csv(self.descriptions_file[1], sep=';')
            augment_texts2 = pd.read_csv(self.descriptions_file[2], sep=';')
    ...
    def load_annotations(self):
        # *** optional ***
        # corrupted_video_root = '/PATH/TO/K400/annotations/corrupted.txt'
        # with open(corrupted_video_root, 'r') as f:
        #     corrupted_video_name_list = f.readlines()
        #     corrupted_video_name_list = [line.rstrip("\n") for line in corrupted_video_name_list]
        #
        # replacement_video_root = '/PATH/TO/K400/annotations/replacement.txt'
        # with open(replacement_video_root, 'r') as f:
        #     replacement_video_name_list = f.readlines()
        #     replacement_video_name_list = [line.rstrip("\n") for line in replacement_video_name_list]
        # *** optional ***
        ...
        with open(self.ann_file, 'r') as fin:
            for line in fin:
                ...
                # *** optional ***
                # if filename.split('/')[-1] in corrupted_video_name_list:
                #     if filename.split('/')[-1] in replacement_video_name_list and 'train.txt' in self.ann_file:
                #         filename = filename.replace('train', 'replacement/replacement_for_corrupted_k400')
                #     else:
                #         continue
                # *** optional ***
...
def build_dataloader(logger, config):
    ...
    train_data = VideoDataset(ann_file=config.DATA.TRAIN_FILE, data_prefix=config.DATA.TRAIN_ROOT,
                              labels_file=config.DATA.LABEL_LIST, descriptions_file=config.DATA.DESCRIPTION,
    ...
    val_data = VideoDataset(ann_file=config.DATA.VAL_FILE, data_prefix=config.DATA.VAL_ROOT,
                            labels_file=config.DATA.LABEL_LIST, descriptions_file=config.DATA.DESCRIPTION,
